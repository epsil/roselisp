...
class Environment {
    ...
    get(key, options = {}) {
        ...
        let [value, found] = this.getTuple(key, inheritedOptions);
        if (found) {
            return value;
        }
        else {
    ...
    getEntry(key, options = {}) {
        const notFound = options['notFound'];
        const inheritedOptions = Object.assign(Object.assign({}, options), { notFound: false });
        let [value, found] = this.getTuple(key, options);
        if (found) {
            return [key, value];
        }
        else {
     ...
     */
    getTuple(key, options = {}) {
        const notFound = options['notFound'];
        const inheritedOptions = Object.assign(Object.assign({}, options), { notFound: false });
        let env = this.findFrame(key, inheritedOptions);
        if (env) {
    ...
    getLocal(key, options = {}) {
        const notFound = options['notFound'];
        const inheritedOptions = Object.assign(Object.assign({}, options), { notFound: false });
        let [value, found] = this.getLocalTuple(key, inheritedOptions);
        if (found) {
            return value;
        }
        else {
     ...
     */
    getLocalTuple(key, options = {}) {
        const notFound = options['notFound'];
        const filter = options['filter'];
        if (filter && !filter(this)) {
            return [notFound, false];
        }
        let found = this.hasLocalP(key);
        const value = found ? this.table.get(key) : notFound;
        return [value, found];
    }
    /**
     ...
     */
    hasp(key, options = {}) {
        let env = this.findFrame(key, options);
        if (env) {
            return true;
        }
        else {
...
class TypedEnvironment extends Environment {
    ...
    getTypedValue(key, options = {
        ...
        return super.get(key, Object.assign(Object.assign({}, options), { notFound: options['notFound'] || [undefined, Symbol.for('Undefined')] }));
    }
    getTypedLocalValue(key, options = {
        ...
        return super.getLocal(key, Object.assign(Object.assign({}, options), { notFound: options['notFound'] || [undefined, Symbol.for('Undefined')] }));
    }
    /**
     ...
     */
    setLocalX(key, value, type = Symbol.for('Any')) {
     ...
     * the current environment frame.
     */
    setTypedValueX(key, value, type = Symbol.for('Any')) {
        let env = this.findFrame(key, {
            notFound: this
        });
...
class EnvironmentStack extends PromiseEnvironment {
    ...
    getFrames(options = {}) {
        ...
        const environments = this.stack;
        const parent = this.parent;
        const inheritedOptions = Object.assign(Object.assign({}, options), { offset: 0 });
        if (parent) {
    ...
    getUnforcedLocalTuple(key, options = {}) {
        const notFound = options['notFound'];
        let env = this.findLocalFrame(key, options);
        if (env) {
            if (env instanceof PromiseEnvironment) {
                return env.getUnforcedTuple(key, options);
            }
...
class EnvironmentPipe extends TypedEnvironment {
     ...
     */
    hasLocalP(key, options = {}) {
        let [value, found] = this.getLocalTuple(key, options);
        return found;
    }
}
...
class DynamicEnvironment extends TypedEnvironment {
     ...
     */
    hasLocalP(key, options = {}) {
        let [, found] = this.getLocalTuple(key, options);
        return found;
    }
}
...
linkEnvironmentFrames.fsource = [Symbol.for('define'), [Symbol.for('link-environment-frames'), Symbol.for('frames')], [Symbol.for('define'), Symbol.for('first-frame'), undefined], [Symbol.for('for'), [[Symbol.for('i'), [Symbol.for('range'), [Symbol.for('-'), [Symbol.for('length'), Symbol.for('frames')], 1], -1, -1]]], [Symbol.for('define'), Symbol.for('frame'), [Symbol.for('list-ref'), Symbol.for('frames'), Symbol.for('i')]], [Symbol.for('set-field!'), Symbol.for('parent'), Symbol.for('frame'), Symbol.for('first-frame')], [Symbol.for('set!'), Symbol.for('first-frame'), Symbol.for('frame')]], Symbol.for('first-frame')];
/**
 * Prefix a set of bindings.
 */
function prefixBindings(prefix, bindings) {
    function prefixBinding(binding) {
 ...
 */
function withEnvironmentF(env, f) {
 ...
 */
function withEnvironment(exp, env) {
...
exports.currentCompilationOptions = currentCompilationOptions;
...
exports.withCompilationOptions = withCompilationOptions;

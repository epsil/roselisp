...
function interpret(exp, env = (0, env_1.defaultEnvironment)(), options = {}) {
    ...
    const inheritedOptions = Object.assign(Object.assign({}, options), { case: 'none', expressionType: expressionType, estree: true, shouldInline: false });
...
function compileSetValues(node, env, options = {}) {
    ...
    return makeStatement(new estree_1.AssignmentExpression('=', left, right), inheritedOptions);
}
...
function compileSetFields(node, env, options = {}) {
    const expressionType = options['expressionType'];
    return makeStatement(new estree_1.AssignmentExpression('=', new estree_1.ObjectPattern(node.get(1).drop(0).map(function (x) {
        let exp = (0, rose_1.roseToSexp)(x);
        if (Array.isArray(exp)) {
        ...
        if (!externalSymbols.includes(symbol) && env1.hasp(symbol)) {
            ...
            if (sourcep(value)) {
                ...
                if ((0, util_1.taggedListP)(exp, Symbol.for('define'))) {
                    ...
                    const compiledExpression = compileRose((0, rose_1.sexpToRose)(exp), env2, Object.assign(Object.assign({}, options), { currentModule: currentModule, referencedSymbols: referencedSymbols1 }));
...
function compileJsFor(node, env, options = {}) {
    ...
    let sym = undefined;
    function bindingp(x) {
        return (x.length === 2) && (typeof x[0] === 'symbol');
    }
    bindingp.fsource = [Symbol.for('define'), [Symbol.for('binding?'), Symbol.for('x')], [Symbol.for('and'), [Symbol.for('='), [Symbol.for('js/length'), Symbol.for('x')], 2], [Symbol.for('symbol?'), [Symbol.for('js/first'), Symbol.for('x')]]]];
    if (bindingp(initExp)) {
        sym = initExp[0];
    ...
    let initCompiled = compileStatement(init, env, options);
    ...
    let testCompiled = compileExpression(test, env, options);
    ...
    let updateCompiled = compileStatement(update, env, options);

...
function compileDefine(node, env, options = {}) {
    ...
    if (Array.isArray((Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && (() => {
        ...
        env.setLocal(nameSym, (0, thunk_1.thunk)(function () {
        ...
        env.setLocal(exp[1], undefined, Symbol.for('Any'));
        ...
        env.setLocal(sym, (0, thunk_1.thunk)(function () {
            let result = undefined;
            try {
...
function compileDefineValues(node, env, options = {}) {
    ...
    if (typeof variables === 'symbol') {
        ...
        env.setLocal(variables, expressionThunk, Symbol.for('Any'));
    }
    else {
        ...
        varDecls = regularVars.map(function (x) {
            if (x === holeMarker) {
                ...
                i++;
                env.setLocal(x, varThunk, Symbol.for('Any'));
        ...
        if (restVar) {
            ...
            i++;
            env.setLocal(restVar, restVarThunk, Symbol.for('Any'));
...
function compileDefineFields(node, env, options = {}) {
    ...
    for (let f of fieldsExp) {
        ...
        env.setLocal(sym, propThunk, Symbol.for('Any'));
    }
...
function makeDefineValuesExp(symbols, env, options) {
    ...
    while (referencedSymbols.length > 0) {
        ...
        seen.push(symbol);
        if (!externalSymbols.includes(symbol) && env1.has(symbol)) {
...
function compileClassHelper(node, env, options = {}) {
    ...
    if (hasName) {
        env.setLocal(className, (0, thunk_1.thunk)(function () {
            let result = undefined;
            try {

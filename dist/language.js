...
function compileJsAssignment(node, env, options = {}) {
    ...
    if ((0, util_1.taggedListP)(left, [Symbol.for('aset!'), Symbol.for('define'), Symbol.for('define-fields'), Symbol.for('define-values'), Symbol.for('oset!'), Symbol.for('set!'), Symbol.for('set!-fields'), Symbol.for('set!-values')])) {
        ...
        if ((0, util_1.taggedListP)(left, [Symbol.for('list'), Symbol.for('values')])) {
            leftCompiled = new estree_1.ArrayPattern(left.drop(1).map(function (x) {
                if ((0, rose_1.syntaxToDatum)(x)) {
        ...
        else if ((0, util_1.taggedListP)(left, Symbol.for('list*'))) {
            const varList = left.drop(1);
            let regularVars = varList.slice(0, -1);
            let restVar = varList[varList.length - 1];
            if (regularVars.length === 0) {
                leftCompiled = (0, rose_1.syntaxToDatum)(restVar) ? compileSymbol(restVar, env, options, {
                    literalSymbol: true
                }) : null;
            }
            else {
                leftCompiled = new estree_1.ArrayPattern([...regularVars.map(function (x) {
                        if ((0, rose_1.syntaxToDatum)(x)) {
                            return compileSymbol(x, env, options, {
                                literalSymbol: true
                            });
                    ...
                    }), new estree_1.RestElement((0, rose_1.syntaxToDatum)(restVar) ? compileSymbol(restVar, env, options, {
                        literalSymbol: true
                    }) : null)]);
            }
        }
...
function compileSetValues(node, env, options = {}) {
    ...
    return compileJsAssignment((0, rose_1.datumToSyntax)(node, [Symbol.for('js/='), left, right]), env, options);
}

...
function compileCond(node, env, options = {}) {
    ...
    function wrap(exps) {
        if (exps.length === 1) {
            ...
            return (0, rose_1.datumToSyntax)(false, [Symbol.for('begin'), ...exps]);
        }
    }
    ...
    const finalExp = (0, util_1.taggedListP)(finalClause, Symbol.for('else')) ? (0, rose_1.datumToSyntax)(false, [(expressionType === 'expression') ? Symbol.for('begin') : Symbol.for('js/block'), ...finalClause.drop(1)]) : (0, rose_1.datumToSyntax)(false, [Symbol.for('if'), finalClause.get(0), wrap(finalClause.drop(1))]);
    const finalExpCompiled = transferAndCompileComments(finalClause, compileSyntax(finalExp, env, options), options);
    let result = clauses.reduceRight(function (alternate, clause) {
        const conditional = compileSyntax((0, rose_1.datumToSyntax)(false, [Symbol.for('if'), clause.get(0), (0, rose_1.transferComments)(clause, wrap(clause.drop(1)))]), env, options);

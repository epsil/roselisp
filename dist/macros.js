...
const [lastCdr, cdr, listStar, cons, take, setCarX] = (() => {
        ...
        if (makeLet && Array.isArray(exp)) {
            ...
            if (Array.isArray(pat) && (pat.length === 0)) {
                ...
                if (pat[pat.length - 1] === Symbol.for('...')) {
                    ...
                    let result = [Symbol.for('and'), [Symbol.for('array?'), exp], [Symbol.for('='), [Symbol.for('js/length'), exp], len]];
                    const _end = pat.length;
                    for (let i = 1; i < _end; i++) {
                        const pat1 = pat[i];
                        const exp1 = [Symbol.for('aget'), exp, i - 1];
                ...
                let result = [Symbol.for('and'), [Symbol.for('array?'), exp], [Symbol.for('>='), [Symbol.for('js/length'), exp], head.length]];
                const _end1 = head.length;
                for (let i = 0; i < _end1; i++) {
                    const pat1 = head[i];
                    const exp1 = [Symbol.for('aget'), exp, i];
                    const result1 = patternMatch(pat1, exp1, false);
                    result = combineExpressions(result, result1);
                }

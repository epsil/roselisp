...
function defun_(exp, env) {
    let [name, args, ...body] = exp.slice(1);
 ...
 */
function defineCompilerMacro_(exp, env) {
    const [nameAndArgs, ...body] = exp.slice(1);
    let name = nameAndArgs[0];
...
exports.defineCompilerMacro_ = defineCompilerMacro_;
...
exports.syntaxMacro_ = syntaxMacro_;
...
function defineMacroToLambdaForm(exp, options = {}) {
    ...
    const args = ((nameAndParams.length === 3) && (nameAndParams[1] === Symbol.for('.'))) ? nameAndParams[2] : nameAndParams.slice(1);
    const body = exp.slice(2);
    let expParam = options['exp'] || Symbol('exp');
    let envParam = options['env'] || Symbol('env');
    let macroParams = [];
    const initializers = [];
    let optional = false;
    ...
    if (Array.isArray(args) && !((args.length >= 3) && (args[args.length - 2] === Symbol.for('.')) && !Array.isArray(args[args.length - 1]))) {
        ...
        while (i < args.length) {
            ...
            if (Array.isArray(arg) || optional) {
                ...
                const param = arg[0];
                const value = arg[1];
                const initializer = [Symbol.for('when'), [Symbol.for('undefined?'), param], [Symbol.for('set!'), param, value]];
 ...
 */
function defsubst_(exp, env) {
    let [name, args, ...body] = exp.slice(1);
    return [Symbol.for('define-inline'), [name, ...args], ...body];
}
exports.defsubst_ = defsubst_;
 ...
 */
function declareSyntaxMacro_(exp, env) {
    let [name] = exp.slice(1);
    if (name === undefined) {
        name = undefined;
    }
...
exports.declareSyntaxMacro_ = declareSyntaxMacro_;
 ...
 */
function while_(stx) {
    return (0, rose_1.datumToSyntax)(stx, [Symbol.for('js/while'), stx.get(1), ...stx.drop(2)]);
}
...
function match_(exp1, env) {
    ...
    if (Array.isArray(exp)) {
        ...
        const lastCondClause = condClauses[condClauses.length - 1];

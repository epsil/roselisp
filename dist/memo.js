...
exports.memoizeWithArg = exports.memoize2 = exports.memoize = exports.eof = void 0;
 ...
 * [w:Memoization]: https://en.wikipedia.org/wiki/Memoization
 */
function memoize(f, cache = new Map()) {
    const [memoizedF] = memoize2(f, cache);
    return memoizedF;
}
 ...
 */
function memoize2(f, cache = new Map()) {
    function memoizedF(...args) {
        const map = getCache(memoizedF);
        const path = makePath(args);
        let [value, hasValue] = (0, util_1.mapGetTuple)(map, path);
        if (hasValue) {
            return value;
        }
        else {
            value = f(...args);
            (0, util_1.mapSetX)(map, path, value);
            return value;
        }
    }
 ...
 * [w:Memoization]: https://en.wikipedia.org/wiki/Memoization
 */
function memoizeWithArg(f, arity = f.length) {
    function memoizedF(...args) {
        const map = args.at(-1);
        if ((args.length === arity) && (map instanceof Map)) {
            const args1 = args.slice(0, -1);
            const path = makePath(args1);
            let [value, hasValue] = (0, util_1.mapGetTuple)(map, path);
            if (hasValue) {
                return value;
            }

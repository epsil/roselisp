...
function sexp(input, ...args) {
    if (input === '') {
        ...
        for (let str of input) {
            str = str.trim();
            if (str !== '') {
                arr = [...arr, ...(0, parser_1.tokenize)(str)];
            }
            if (args.length > 0) {
                arr.push(new parser_1.Token(args.shift()));
            }
        }
        if (Array.isArray(arr) && (arr.length === 0)) {
            ...
            return (0, parser_1.parseSexp)(arr);
        }
    }
    else {

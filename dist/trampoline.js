...
class Trampoline {
    ...
    constructor(f = undefined, ...args) {
        ...
        if (f) {
            const initialCall = new TrampolineCall(f, ...args);
...
class TrampolineCall {
    ...
    mapRight(f) {
        const call = [];
        const _start = this.size() - 1;
        for (let i = _start; i > -1; i--) {
            call.unshift(f(this.call[i]));
        }
...
exports.TrampolineCall = TrampolineCall;
/**
...
function trampoline(f, ...args) {
    const trampolineInstance = new Trampoline(f, ...args);

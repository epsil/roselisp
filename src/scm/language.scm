...
(define compilation-compiler-mapping-env
  (new CompilationEnvironment
       `((,add_ ,compile-add (->compiler :rest Any Any))
         (,and_ ,compile-and (->compiler :rest Any Any))
         (,ann_ ,compile-ann (->compiler :rest Any Any))
         (,append_ ,compile-append (->compiler :rest Any Any))
         (,apply_ ,compile-apply (->compiler :rest Any Any))
         (,array-ref_ ,compile-array-ref (->compiler :rest Any Any))
         (,array-set_ ,compile-array-set (->compiler :rest Any Any))
         (,begin_ ,compile-begin (->compiler :rest Any Any))
         (,block_ ,compile-block (->compiler :rest Any Any))
         (,break_ ,compile-break (->compiler :rest Any Any))
         (,class_ ,compile-class (->compiler :rest Any Any))
         (,colon_ ,compile-colon (->compiler :rest Any Any))
         (,cond_ ,compile-cond (->compiler :rest Any Any))
         (,continue_ ,compile-continue (->compiler :rest Any Any))
         (,declare-macro_ ,compile-declare-macro (->compiler :rest Any Any))
         (,define-async_ ,compile-define-async (->compiler :rest Any Any))
         (,define-class_ ,compile-define-class (->compiler :rest Any Any))
         (,define-fields_ ,compile-define-fields (->compiler :rest Any Any))
         (,define-generator_ ,compile-define-generator (->compiler :rest Any Any))
         (,define-macro_ ,compile-define-macro (->compiler :rest Any Any))
         (,define-type_ ,compile-define-type (->compiler :rest Any Any))
         (,define-values_ ,compile-define-values (->compiler :rest Any Any))
         (,define_ ,compile-define (->compiler :rest Any Any))
         (,div_ ,compile-div (->compiler :rest Any Any))
         (,dot_ ,compile-send (->compiler :rest Any Any))
         (,for_ ,compile-for (->compiler :rest Any Any))
         (,funcall_ ,compile-funcall (->compiler :rest Any Any))
         (,get-field_ ,compile-get-field (->compiler :rest Any Any))
         (,gt_ ,compile-greater-than (->compiler :rest Any Any))
         (,gte_ ,compile-greater-than-or-equal (->compiler :rest Any Any))
         (,js-arrow_ ,compile-js-arrow (->compiler :rest Any Any))
         (,js-async_ ,compile-js-async (->compiler :rest Any Any))
         (,js-await_ ,compile-js-await (->compiler :rest Any Any))
         (,js-delete_ ,compile-js-delete (->compiler :rest Any Any))
         ...
         (,js-plus_ ,compile-add (->compiler :rest Any Any))
         ...
         (,let-fields_ ,compile-let-fields (->compiler :rest Any Any))
         (,let-star_ ,compile-let (->compiler :rest Any Any))
         (,let-values_ ,compile-let-values (->compiler :rest Any Any))
         (,list_ ,compile-list (->compiler :rest Any Any))
         (,lt_ ,compile-less-than (->compiler :rest Any Any))
         (,lte_ ,compile-less-than-or-equal (->compiler :rest Any Any))
         (,module_ ,compile-module (->compiler :rest Any Any))
         (,modulo_ ,compile-modulo (->compiler :rest Any Any))
         ...
         (,push-left!_ ,compile-push-left (->compiler :rest Any Any))
         (,push-right!_ ,compile-push-right (->compiler :rest Any Any))
         (,quasiquote_ ,compile-quasiquote (->compiler :rest Any Any))
         (,quote_ ,compile-quote (->compiler :rest Any Any))
         (,require_ ,compile-require (->compiler :rest Any Any))
         (,return_ ,compile-return (->compiler :rest Any Any))
         (,send-apply_ ,compile-send-apply (->compiler :rest Any Any))
         (,send_ ,compile-send (->compiler :rest Any Any))
         (,set!_ ,compile-set (->compiler :rest Any Any))
         (,set-field_ ,compile-set-field (->compiler :rest Any Any))
         (,set-fields_ ,compile-set-fields (->compiler :rest Any Any))
         (,set-values_ ,compile-set-values (->compiler :rest Any Any))
         (,string-append_ ,compile-string-append (->compiler :rest Any Any))
         (,sub_ ,compile-sub (->compiler :rest Any Any))
         (,throw_ ,compile-throw (->compiler :rest Any Any))
         (,yield_ ,compile-yield (->compiler :rest Any Any)))))

;;; Compiler macros mapping environment.
(define compilation-macro-mapping-env
  (new CompilationEnvironment
       `((,array-drop-right_ ,compile-array-drop-right-macro (->macro :rest Any Any))
         (,array-drop_ ,compile-array-drop-macro (->macro :rest Any Any))
         (,array-list-drop-right_ ,compile-array-list-drop-right-macro (->macro :rest Any Any))
         (,array-list-drop_ ,compile-array-list-drop-macro (->macro :rest Any Any))
         (,assert_ ,compile-assert-macro (->macro :rest Any Any))
         (,display_ ,compile-display-macro (->macro :rest Any Any))
         (,drop-right_ ,compile-drop-right-macro (->macro :rest Any Any))
         (,drop_ ,compile-drop-macro (->macro :rest Any Any))
         (,foldl_ ,compile-foldl-macro (->macro :rest Any Any))
         (,foldr_ ,compile-foldr-macro (->macro :rest Any Any))
         (,hash-clear_ ,compile-hash-clear-macro (->macro :rest Any Any))
         (,hash-ref_ ,compile-hash-ref-macro (->macro :rest Any Any))
         (,hash-remove!_ ,compile-hash-remove-macro (->macro :rest Any Any))
         (,hash-remove_ ,compile-hash-remove-macro (->macro :rest Any Any))
         (,make-hash_ ,compile-make-hash-macro (->macro :rest Any Any))
         (,map_ ,compile-map-macro (->macro :rest Any Any))
         (,member?_ ,compile-member-p-macro (->macro :rest Any Any))
         (,print ,compile-display-macro (->macro :rest Any Any))
         (,regexp_ ,compile-regexp-macro (->macro :rest Any Any))
         (,string-trim_ ,compile-string-trim-macro (->macro :rest Any Any))
         (,string?_ ,compile-stringp-macro (->macro :rest Any Any))
         (,substring_ ,compile-substring-macro (->macro :rest Any Any))
         (,values_ ,compile-values-macro (->macro :rest Any Any)))))
     ...
     ((or (eq? declared-type #u)
      ...
      (set! type_
            `(->
              ...
              ,return-type))
      (send continuation-env set-local name-sym type_ '(->* :rest Any Any)))
     (else
  ...
  (for ((i (range 0 (array-list-length body))))
    ...
    (cond
     ...
     ((form? exp define-macro_ env)
      (define sym
        (first (second exp)))
      (send continuation-env set-local sym #t '(->macro :rest Any Any)))
...
(defmacro define-macro_ (&whole exp &environment env)
  ...
  (define f
    (eval_ f-exp env))
  (send env set name f '(->macro :rest Any Any))
  f-exp)
...
(define (map-visit-rose f
  ...
  (define (visit-define node stack bindings)
    ...
    (cond
     ((array? id-exp)
      (send bindings set-local id-sym #t '(->* :rest Any Any))
  ...
  (define (visit-defmacro node stack bindings)
    ...
    (send bindings set-local id-sym #t '(->macro :rest Any Any))
    (define bindings-2
      (extend-environment (new LispEnvironment) bindings))
    (cond
     ((symbol? params-exp)
...
(define lisp-environment
  (new LispEnvironment
       `(
         ...
         (license ,license Any)
         ;; Procedures.
         ($ ,funcall_ (->* :rest Any Any))
         (% ,modulo_ (->* :rest Any Any))
         (* ,mul_ (->* :rest Any Any))
         (+ ,add_ (->* :rest Any Any))
         (- ,sub_ (->* :rest Any Any))
         (/ ,div_ (->* :rest Any Any))
         ...
         (> ,gt_ (->* :rest Any Any))
         (>= ,gte_ (->* :rest Any Any))
         (add ,add_ (->* :rest Any Any))
         (add1 ,add1_ (->* :rest Any Any))
         (aget ,array-ref_ (->* :rest Any Any))
         (append ,append_ (->* :rest Any Any))
         (apply ,apply_ (->* :rest Any Any))
         (aref ,array-ref_ (->* :rest Any Any))
         (array-drop ,array-drop_ (->* :rest Any Any))
         (array-drop-right ,array-drop-right_ (->* :rest Any Any))
         (array-eighth ,array-eighth_ (->* :rest Any Any))
         (array-fifth ,array-fifth_ (->* :rest Any Any))
         (array-first ,array-first_ (->* :rest Any Any))
         (array-fourth ,array-fourth_ (->* :rest Any Any))
         (array-get ,array-ref_ (->* :rest Any Any))
         (array-last ,array-last_ (->* :rest Any Any))
         (array-length ,array-length_ (->* :rest Any Any))
         (array-list->linked-list ,array-list->linked-list_ (->* :rest Any Any))
         (array-list-car ,car_ (->* :rest Any Any))
         (array-list-cdr ,array-list-cdr_ (->* :rest Any Any))
         (array-list-drop ,array-list-drop_ (->* :rest Any Any))
         (array-list-drop-right ,array-list-drop-right_ (->* :rest Any Any))
         (array-list-eighth ,array-list-eighth_ (->* :rest Any Any))
         (array-list-fifth ,array-list-fifth_ (->* :rest Any Any))
         (array-list-first ,array-list-first_ (->* :rest Any Any))
         (array-list-fourth ,array-list-fourth_ (->* :rest Any Any))
         (array-list-last ,array-list-last_ (->* :rest Any Any))
         (array-list-length ,array-list-length_ (->* :rest Any Any))
         (array-list-ninth ,array-list-ninth_ (->* :rest Any Any))
         (array-list-nth ,array-list-nth_ (->* :rest Any Any))
         (array-list-nthcdr ,array-list-nthcdr_ (->* :rest Any Any))
         (array-list-rest ,array-list-rest_ (->* :rest Any Any))
         (array-list-reverse ,array-list-reverse_ (->* :rest Any Any))
         (array-list-second ,array-list-second_ (->* :rest Any Any))
         (array-list-seventh ,array-list-seventh_ (->* :rest Any Any))
         (array-list-sixth ,array-list-sixth_ (->* :rest Any Any))
         (array-list-take ,array-list-take_ (->* :rest Any Any))
         (array-list-tenth ,array-list-tenth_ (->* :rest Any Any))
         (array-list-third ,array-list-third_ (->* :rest Any Any))
         (array-list? ,array-list?_ (->* :rest Any Any))
         (array-ninth ,array-ninth_ (->* :rest Any Any))
         (array-ref ,array-ref_ (->* :rest Any Any))
         (array-rest ,array-rest_ (->* :rest Any Any))
         (array-reverse ,array-reverse_ (->* :rest Any Any))
         (array-second ,array-second_ (->* :rest Any Any))
         (array-set ,array-set_ (->* :rest Any Any))
         (array-set! ,array-set_ (->* :rest Any Any))
         (array-seventh ,array-seventh_ (->* :rest Any Any))
         (array-sixth ,array-sixth_ (->* :rest Any Any))
         (array-take ,array-take_ (->* :rest Any Any))
         (array-tenth ,array-tenth_ (->* :rest Any Any))
         (array-third ,array-third_ (->* :rest Any Any))
         (array? ,array?_ (->* :rest Any Any))
         (aset ,array-set_ (->* :rest Any Any))
         (aset! ,array-set_ (->* :rest Any Any))
         (assert ,assert_ (->* :rest Any Any))
         (boolean? ,boolean?_ (->* :rest Any Any))
         (booleanp ,boolean?_ (->* :rest Any Any))
         (build-list ,build-list_ (->* :rest Any Any))
         (cadr ,cadr_ (->* :rest Any Any))
         (call-cc ,call-with-current-continuation_ (->* :rest Any Any))
         (call-with-current-continuation ,call-with-current-continuation_ (->* :rest Any Any))
         (call/cc ,call-with-current-continuation_ (->* :rest Any Any))
         (car ,car_ (->* :rest Any Any))
         (cdr ,cdr_ (->* :rest Any Any))
         (circular-list-p ,circular-list?_ (->* :rest Any Any))
         (circular-list? ,circular-list?_ (->* :rest Any Any))
         (cons ,cons_ (->* :rest Any Any))
         (cons* ,list-star_ (->* :rest Any Any))
         (cons-dot ,cons-dot-f_ (->* :rest Any Any))
         (cons-dot? ,cons-dot?_ (->* :rest Any Any))
         (cons? ,cons?_ (->* :rest Any Any))
         (console.log ,(get-field log console) (->* :rest Any Any))
         (consp ,cons?_ (->* :rest Any Any))
         (const ,const_ (->* :rest Any Any))
         (constantly ,const_ (->* :rest Any Any))
         (current-environment ,current-environment_ (->* :rest Any Any))
         (curry ,curry (->* :rest Any Any))
         (curry-n ,curry-n (->* :rest Any Any))
         (delete ,js-delete_ (->* :rest Any Any))
         (display ,display_ (->* :rest Any Any))
         (div ,div_ (->* :rest Any Any))
         (dotted-list->proper-list ,linked-list->array-list_ (->* :rest Any Any))
         (dotted-list-car ,linked-list-car_ (->* :rest Any Any))
         (dotted-list-cdr ,linked-list-cdr_ (->* :rest Any Any))
         (dotted-list-head ,linked-list-head_ (->* :rest Any Any))
         (dotted-list-last ,linked-list-last_ (->* :rest Any Any))
         (dotted-list-last-cdr ,linked-list-last-cdr_ (->* :rest Any Any))
         (dotted-list-length ,linked-list-length_ (->* :rest Any Any))
         (dotted-list-nth ,linked-list-nth_ (->* :rest Any Any))
         (dotted-list-nthcdr ,linked-list-nthcdr_ (->* :rest Any Any))
         (dotted-list-p ,dotted-list?_ (->* :rest Any Any))
         (dotted-list-tail ,linked-list-tail_ (->* :rest Any Any))
         (dotted-list? ,dotted-list?_ (->* :rest Any Any))
         (dotted-pair-cdr ,linked-pair-cdr_ (->* :rest Any Any))
         (dotted-pair-p ,dotted-pair-p_ (->* :rest Any Any))
         (dotted-pair? ,dotted-pair-p_ (->* :rest Any Any))
         (drop ,list-tail_ (->* :rest Any Any))
         (drop-right ,drop-right_ (->* :rest Any Any))
         (eighth ,eighth_ (->* :rest Any Any))
         (eq ,eq?_ (->* :rest Any Any))
         (eq? ,eq?_ (->* :rest Any Any))
         (eql ,eqv?_ (->* :rest Any Any))
         (eql? ,eqv?_ (->* :rest Any Any))
         (equal ,equal?_ (->* :rest Any Any))
         (equal? ,equal?_ (->* :rest Any Any))
         (eqv ,eqv?_ (->* :rest Any Any))
         (eqv? ,eqv?_ (->* :rest Any Any))
         (error ,error_ (->* :rest Any Any))
         (even? ,even?_ (->* :rest Any Any))
         (extend-environment ,extend-environment (->* :rest Any Any))
         (false? ,false?_ (->* :rest Any Any))
         (falsep ,false?_ (->* :rest Any Any))
         (fexpr? ,fexpr?_ (->* :rest Any Any))
         (fexprp ,fexpr?_ (->* :rest Any Any))
         (field-names ,field-names_ (->* :rest Any Any))
         (fifth ,fifth_ (->* :rest Any Any))
         (filter ,filter_ (->* :rest Any Any))
         (findf ,findf_ (->* :rest Any Any))
         (findf-index ,findf-index_ (->* :rest Any Any))
         (first ,first_ (->* :rest Any Any))
         (flatten ,flatten_ (->* :rest Any Any))
         (foldl ,foldl_ (->* :rest Any Any))
         (foldr ,foldr_ (->* :rest Any Any))
         (fourth ,fourth_ (->* :rest Any Any))
         (funcall ,funcall_ (->* :rest Any Any))
         (function-object? ,js-function-object?_ (->* :rest Any Any))
         (function-type? ,js-function-type?_ (->* :rest Any Any))
         (function? ,procedure?_ (->* :rest Any Any))
         (functionp ,procedure?_ (->* :rest Any Any))
         (gensym ,gensym_ (->* :rest Any Any))
         (gensym? ,gensym?_ (->* :rest Any Any))
         (get ,array-ref_ (->* :rest Any Any))
         (hash ,make-hash_ (->* :rest Any Any))
         (hash->list ,hash->list_ (->* :rest Any Any))
         (hash-clear ,hash-clear_ (->* :rest Any Any))
         (hash-clear! ,hash-clear!_ (->* :rest Any Any))
         (hash-copy ,hash-copy_ (->* :rest Any Any))
         (hash-entries ,hash-entries_ (->* :rest Any Any))
         (hash-has-key? ,hash-has-key?_ (->* :rest Any Any))
         (hash-keys ,hash-keys_ (->* :rest Any Any))
         (hash-ref ,hash-ref_ (->* :rest Any Any))
         (hash-remove ,hash-remove_ (->* :rest Any Any))
         (hash-remove! ,hash-remove!_ (->* :rest Any Any))
         (hash-set ,hash-set!_ (->* :rest Any Any))
         (hash-set! ,hash-set!_ (->* :rest Any Any))
         (hash-size ,hash-size_ (->* :rest Any Any))
         (hash-values ,hash-values_ (->* :rest Any Any))
         (hash? ,hash?_ (->* :rest Any Any))
         (head ,car_ (->* :rest Any Any))
         (id ,identity_ (->* :rest Any Any))
         (identity ,identity_ (->* :rest Any Any))
         (improper-list-p ,improper-list?_ (->* :rest Any Any))
         (improper-list? ,improper-list?_ (->* :rest Any Any))
         (in-range ,range_ (->* :rest Any Any))
         (index-of ,index-of_ (->* :rest Any Any))
         (index-where ,index-where_ (->* :rest Any Any))
         (instance-of ,is-a?_ (->* :rest Any Any))
         (instance-of? ,is-a?_ (->* :rest Any Any))
         (instanceof ,is-a?_ (->* :rest Any Any))
         (instanceof? ,is-a?_ (->* :rest Any Any))
         (intern ,string->symbol_ (->* :rest Any Any))
         (intersection ,intersection_ (->* :rest Any Any))
         (is-a? ,is-a?_ (->* :rest Any Any))
         (js ,js_ (->* :rest Any Any))
         ...
         (last-cdr ,last-cdr_ (->* :rest Any Any))
         (last-cons ,last-pair_ (->* :rest Any Any))
         (last-pair ,last-pair_ (->* :rest Any Any))
         (length ,length_ (->* :rest Any Any))
         (length* ,length_ (->* :rest Any Any))
         (linked-list-car ,linked-list-car_ (->* :rest Any Any))
         (linked-list-cdr ,linked-list-cdr_ (->* :rest Any Any))
         (linked-list-eighth ,linked-list-eighth_ (->* :rest Any Any))
         (linked-list-fifth ,linked-list-fifth_ (->* :rest Any Any))
         (linked-list-first ,linked-list-first_ (->* :rest Any Any))
         (linked-list-fourth ,linked-list-fourth_ (->* :rest Any Any))
         (linked-list-head ,linked-list-head_ (->* :rest Any Any))
         (linked-list-last ,linked-list-last_ (->* :rest Any Any))
         (linked-list-last-cdr ,linked-list-last-cdr_ (->* :rest Any Any))
         (linked-list-length ,linked-list-length_ (->* :rest Any Any))
         (linked-list-link-car ,linked-list-link-car_ (->* :rest Any Any))
         (linked-list-link-cdr ,linked-list-link-cdr_ (->* :rest Any Any))
         (linked-list-link-p ,linked-list-link?_ (->* :rest Any Any))
         (linked-list-link? ,linked-list-link?_ (->* :rest Any Any))
         (linked-list-ninth ,linked-list-ninth_ (->* :rest Any Any))
         (linked-list-nth ,linked-list-nth_ (->* :rest Any Any))
         (linked-list-nthcdr ,linked-list-nthcdr_ (->* :rest Any Any))
         (linked-list-p ,linked-list?_ (->* :rest Any Any))
         (linked-list-second ,linked-list-second_ (->* :rest Any Any))
         (linked-list-seventh ,linked-list-seventh_ (->* :rest Any Any))
         (linked-list-sixth ,linked-list-sixth_ (->* :rest Any Any))
         (linked-list-tail ,linked-list-tail_ (->* :rest Any Any))
         (linked-list-tenth ,linked-list-tenth_ (->* :rest Any Any))
         (linked-list-third ,linked-list-third_ (->* :rest Any Any))
         (linked-list? ,linked-list?_ (->* :rest Any Any))
         (linked-pair-car ,linked-pair-car_ (->* :rest Any Any))
         (linked-pair-cdr ,linked-pair-cdr_ (->* :rest Any Any))
         (linked-pair? ,linked-pair?_ (->* :rest Any Any))
         (list ,list_ (->* :rest Any Any))
         (list* ,list-star_ (->* :rest Any Any))
         (list-ref ,nth_ (->* :rest Any Any))
         (list-set ,array-set_ (->* :rest Any Any))
         (list-set! ,array-set_ (->* :rest Any Any))
         (list-star ,list-star_ (->* :rest Any Any))
         (list-tail ,list-tail_ (->* :rest Any Any))
         ...
         (make-object ,new_ (->* :rest Any Any))
         (map ,map_ (->* :rest Any Any))
         (mapcar ,map_ (->* :rest Any Any))
         (member ,member_ (->* :rest Any Any))
         (member-p ,member?_ (->* :rest Any Any))
         (member? ,member?_ (->* :rest Any Any))
         (memberp ,member?_ (->* :rest Any Any))
         (memf ,memf_ (->* :rest Any Any))
         (memf? ,memf?_ (->* :rest Any Any))
         (memq ,memq_ (->* :rest Any Any))
         (memq? ,memq?_ (->* :rest Any Any))
         (mod ,modulo_ (->* :rest Any Any))
         (modulo ,modulo_ (->* :rest Any Any))
         (mul ,mul_ (->* :rest Any Any))
         (new ,new_ (->* :rest Any Any))
         (new* ,new_ (->* :rest Any Any))
         (ninth ,ninth_ (->* :rest Any Any))
         (not ,not_ (->* :rest Any Any))
         (nth ,nth_ (->* :rest Any Any))
         (nthcdr ,nthcdr_ (->* :rest Any Any))
         (null? ,null?_ (->* :rest Any Any))
         (nullp ,null?_ (->* :rest Any Any))
         (number->string ,number->string_ (->* :rest Any Any))
         (number? ,number?_ (->* :rest Any Any))
         (numberp ,number?_ (->* :rest Any Any))
         (object? ,js-obj-p_ (->* :rest Any Any))
         (objectp ,js-obj-p_ (->* :rest Any Any))
         (odd? ,odd?_ (->* :rest Any Any))
         (oget ,object-ref_ (->* :rest Any Any))
         (one? ,one?_ (->* :rest Any Any))
         (onep ,one?_ (->* :rest Any Any))
         (oref ,array-ref_ (->* :rest Any Any))
         (oset ,object-set!_ (->* :rest Any Any))
         (oset! ,object-set!_ (->* :rest Any Any))
         (plist->alist ,plist->alist_ (->* :rest Any Any))
         (plist-copy ,plist-copy_ (->* :rest Any Any))
         (plist-get ,plist-get_ (->* :rest Any Any))
         (plist-has ,plist-has?_ (->* :rest Any Any))
         (plist-has? ,plist-has?_ (->* :rest Any Any))
         (plist-ref ,plist-get_ (->* :rest Any Any))
         (plist-set ,plist-set!_ (->* :rest Any Any))
         (plist-set! ,plist-set!_ (->* :rest Any Any))
         (plist? ,plist?_ (->* :rest Any Any))
         (pop ,pop-left!_ (->* :rest Any Any))
         (pop! ,pop-left!_ (->* :rest Any Any))
         (pop-left ,pop-left!_ (->* :rest Any Any))
         (pop-left! ,pop-left!_ (->* :rest Any Any))
         (pop-right ,pop-right!_ (->* :rest Any Any))
         (pop-right! ,pop-right!_ (->* :rest Any Any))
         (print ,print (->* :rest Any Any))
         (print-estree ,print-estree (->* :rest Any Any))
         (procedure? ,procedure?_ (->* :rest Any Any))
         (proper-list->dotted-list ,array-list->linked-list_ (->* :rest Any Any))
         (proper-list-p ,proper-list?_ (->* :rest Any Any))
         (proper-list? ,proper-list?_ (->* :rest Any Any))
         (push ,push-left!_ (->* :rest Any Any))
         (push! ,push-left!_ (->* :rest Any Any))
         (push-left ,push-left!_ (->* :rest Any Any))
         (push-left! ,push-left!_ (->* :rest Any Any))
         (push-right ,push-right!_ (->* :rest Any Any))
         (push-right! ,push-right!_ (->* :rest Any Any))
         (range ,range_ (->* :rest Any Any))
         (re ,regexp_ (->* :rest Any Any))
         (re-pattern ,regexp_ (->* :rest Any Any))
         (regexp ,regexp_ (->* :rest Any Any))
         (regexp-match ,regexp-match_ (->* :rest Any Any))
         (regexp-match? ,regexp-match?_ (->* :rest Any Any))
         (regexp-quote ,regexp-quote_ (->* :rest Any Any))
         (regexp-replace ,regexp-replace_ (->* :rest Any Any))
         (regexp? ,regexp?_ (->* :rest Any Any))
         (rest ,rest_ (->* :rest Any Any))
         (reverse ,reverse_ (->* :rest Any Any))
         (rx ,regexp_ (->* :rest Any Any))
         (scm/new ,new_ (->* :rest Any Any))
         (second ,second_ (->* :rest Any Any))
         (self-evaluating? ,self-evaluating?_ (->* :rest Any Any))
         (set-car! ,set-car!_ (->* :rest Any Any))
         (set-cdr! ,set-cdr!_ (->* :rest Any Any))
         (set-mcar! ,set-car!_ (->* :rest Any Any))
         (set-mcdr! ,set-cdr!_ (->* :rest Any Any))
         (set-nth ,array-set_ (->* :rest Any Any))
         (set-nth! ,array-set_ (->* :rest Any Any))
         (seventh ,seventh_ (->* :rest Any Any))
         (sixth ,sixth_ (->* :rest Any Any))
         (source ,source (->* :rest Any Any))
         (string->number ,string->number_ (->* :rest Any Any))
         (string->symbol ,string->symbol_ (->* :rest Any Any))
         (string-append ,string-append_ (->* :rest Any Any))
         (string-downcase ,string-downcase_ (->* :rest Any Any))
         (string-join ,string-join_ (->* :rest Any Any))
         (string-length ,length_ (->* :rest Any Any))
         (string-object? ,string-object?_ (->* :rest Any Any))
         (string-primitive? ,string-primitive?_ (->* :rest Any Any))
         (string-ref ,string-ref_ (->* :rest Any Any))
         (string-repeat ,string-repeat_ (->* :rest Any Any))
         (string-replace ,string-replace_ (->* :rest Any Any))
         (string-split ,string-split_ (->* :rest Any Any))
         (string-to-symbol ,string->symbol_ (->* :rest Any Any))
         (string-trim ,string-trim_ (->* :rest Any Any))
         (string-upcase ,string-upcase_ (->* :rest Any Any))
         (string? ,string?_ (->* :rest Any Any))
         (stringp ,string?_ (->* :rest Any Any))
         (sub ,sub_ (->* :rest Any Any))
         (sub1 ,sub1_ (->* :rest Any Any))
         (substring ,substring_ (->* :rest Any Any))
         (symbol->string ,symbol->string_ (->* :rest Any Any))
         (symbol-to-string ,symbol->string_ (->* :rest Any Any))
         (symbol? ,symbol?_ (->* :rest Any Any))
         (symbolp ,symbol?_ (->* :rest Any Any))
         (tail ,cdr_ (->* :rest Any Any))
         (take ,take_ (->* :rest Any Any))
         (tenth ,tenth_ (->* :rest Any Any))
         (third ,third_ (->* :rest Any Any))
         (true? ,true?_ (->* :rest Any Any))
         (truep ,true?_ (->* :rest Any Any))
         (type-of ,type-of_ (->* :rest Any Any))
         (typeof ,type-of_ (->* :rest Any Any))
         (undefined? ,undefined?_ (->* :rest Any Any))
         (union ,union_ (->* :rest Any Any))
         (values ,values_ (->* :rest Any Any))
         (vector ,list_ (->* :rest Any Any))
         (vector-ref ,nth_ (->* :rest Any Any))
         (vector-set ,array-set_ (->* :rest Any Any))
         (vector-set! ,array-set_ (->* :rest Any Any))
         (vector? ,array?_ (->* :rest Any Any))
         (zero? ,zero?_ (->* :rest Any Any))
         (zerop ,zero?_ (->* :rest Any Any))
         ;; Macros.
         (,(string->symbol ".") ,dot_ (->macro :rest Any Any))
         (,(string->symbol ":") ,colon_ (->macro :rest Any Any))
         (,quasiquote-sym_ ,quasiquote_ (->macro :rest Any Any))
         (,quote-sym_ ,quote_ (->macro :rest Any Any))
         (-> ,thread-first_ (->macro :rest Any Any))
         (->> ,thread-last_ (->macro :rest Any Any))
         (~> ,thread-first_ (->macro :rest Any Any))
         (~>> ,thread-last_ (->macro :rest Any Any))
         (and ,and_ (->macro :rest Any Any))
         (ann ,ann_ (->macro :rest Any Any))
         (as-> ,thread-as_ (->macro :rest Any Any))
         (async ,js-async_ (->macro :rest Any Any))
         (as~> ,thread-as_ (->macro :rest Any Any))
         (await ,js-await_ (->macro :rest Any Any))
         (begin ,begin_ (->macro :rest Any Any))
         (begin0 ,begin0_ (->macro :rest Any Any))
         (block ,block_ (->macro :rest Any Any))
         (break ,break_ (->macro :rest Any Any))
         (call-method ,send_ (->macro :rest Any Any))
         (case ,case_ (->macro :rest Any Any))
         (case/eq ,case-eq_ (->macro :rest Any Any))
         (class ,class_ (->macro :rest Any Any))
         ...
         (define-class ,define-class_ (->macro :rest Any Any))
         (define-fields ,define-fields_ (->macro :rest Any Any))
         (define-js-obj ,define-fields_ (->macro :rest Any Any))
         (define-macro ,define-macro_ (->macro :rest Any Any))
         (define-type ,define-type_ (->macro :rest Any Any))
         (define-values ,define-values_ (->macro :rest Any Any))
         (define/async ,define-async_ (->macro :rest Any Any))
         (define/generator ,define-generator_ (->macro :rest Any Any))
         (define/private ,define-private_ (->macro :rest Any Any))
         (define/public ,define-public_ (->macro :rest Any Any))
         (defmacro ,defmacro_ (->macro :rest Any Any))
         (defun ,defun_ (->macro :rest Any Any))
         (destructuring-bind ,multiple-value-bind_ (->macro :rest Any Any))
         (do ,do_ (->macro :rest Any Any))
         (field-bound? ,field-bound?_ (->macro :rest Any Any))
         (fn ,lambda_ (->macro :rest Any Any))
         (for ,for_ (->macro :rest Any Any))
         (fset ,set_ (->macro :rest Any Any))
         (get-field ,get-field_ (->macro :rest Any Any))
         (if ,if_ (->macro :rest Any Any))
         (js/?. ,js-optional-chaining_ (->macro :rest Any Any))
         (js/arrow ,js-arrow_ (->macro :rest Any Any))
         (js/async ,js-async_ (->macro :rest Any Any))
         (js/await ,js-await_ (->macro :rest Any Any))
         (js/do-while ,js-do-while_ (->macro :rest Any Any))
         (js/for ,js/for_ (->macro :rest Any Any))
         (js/for-in ,js/for-in_ (->macro :rest Any Any))
         (js/for-of ,js/for-of_ (->macro :rest Any Any))
         (js/function ,js-function_ (->macro :rest Any Any))
         (js/switch ,js-switch_ (->macro :rest Any Any))
         (js/try ,js-try_ (->macro :rest Any Any))
         (js/while ,js-while_ (->macro :rest Any Any))
         (λ ,lambda_ (->macro :rest Any Any))
         (lambda ,lambda_ (->macro :rest Any Any))
         (let ,let-star_ (->macro :rest Any Any))
         (let* ,let-star_ (->macro :rest Any Any))
         (let*-values ,let-values_ (->macro :rest Any Any))
         (let-env ,let-env_ (->macro :rest Any Any))
         (let-fields ,let-fields_ (->macro :rest Any Any))
         (let-js-obj ,let-fields_ (->macro :rest Any Any))
         (let-values ,let-values_ (->macro :rest Any Any))
         (letrec ,let-star_ (->macro :rest Any Any))
         (letrec-values ,let-values_ (->macro :rest Any Any))
         (module ,module_ (->macro :rest Any Any))
         (multiple-value-bind ,multiple-value-bind_ (->macro :rest Any Any))
         (multiple-values-bind ,multiple-value-bind_ (->macro :rest Any Any))
         (new/apply ,new-apply_ (->macro :rest Any Any))
         (or ,or_ (->macro :rest Any Any))
         (prog1 ,begin0_ (->macro :rest Any Any))
         (progn ,begin_ (->macro :rest Any Any))
         (provide ,provide_ (->macro :rest Any Any))
         (require ,require_ (->macro :rest Any Any))
         (return ,return_ (->macro :rest Any Any))
         (rkt/new ,rkt-new_ (->macro :rest Any Any))
         (send ,send_ (->macro :rest Any Any))
         (send/apply ,send-apply_ (->macro :rest Any Any))
         (set ,set_ (->macro :rest Any Any))
         (set! ,set!_ (->macro :rest Any Any))
         (set!-fields ,set-fields_ (->macro :rest Any Any))
         (set!-js-obj ,set-fields_ (->macro :rest Any Any))
         (set!-values ,set-values_ (->macro :rest Any Any))
         (set-field! ,set-field_ (->macro :rest Any Any))
         (setq ,set!_ (->macro :rest Any Any))
         (throw ,throw_ (->macro :rest Any Any))
         (try ,clj-try_ (->macro :rest Any Any))
         (unless ,unless_ (->macro :rest Any Any))
         (unwind-protect ,unwind-protect_ (->macro :rest Any Any))
         (when ,when_ (->macro :rest Any Any))
         (while ,while_ (->macro :rest Any Any))
         (yield ,yield_ (->macro :rest Any Any)))))

;;; Evaluation environment.
(define eval-environment
  (new LispEnvironment
       `((eval ,interpret (->* :rest Any Any))
         (js/eval ,js-eval_ (->* :rest Any Any))
         (scm/eval ,interpret (->* :rest Any Any))
         (seval ,eval_ (->* :rest Any Any)))))

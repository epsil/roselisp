      ...
      return ((): any => {
        function linkedListLast_(lst: any): any {
          ...
          while (Array.isArray(current) && (current.length >= 3) && (current[current.length - 2] === Symbol.for('.')) && !((): any => {
            let x: any = current[current.length - 1];
...
function defineToLambda(node: any, options: any = {}): any {
  ...
  if (shouldCurry) {
    ...
    if (Array.isArray(nameAndParams) && (nameAndParams.length >= 3) && (nameAndParams[nameAndParams.length - 2] === Symbol.for('.')) && !((): any => {
      let x: any = lastCdr(nameAndParams);
      return Array.isArray(x) && (x.length === 0);
    })() && (params.length === 1)) {
      params = params[0];
    }
    ...
    if (shouldMakeLet) {
      ...
      for (let i: any = 0; i < _end2; i++) {
        const argExp: any = (i < argsList.length) ? (argsList as any)[i] : ((): any => {
          ...
          if (Array.isArray(currentParam)) {
            if (Array.isArray(currentParam) && (currentParam.length >= 3) && (currentParam[currentParam.length - 2] === Symbol.for('.')) && ((): any => {
              let x: any = lastCdr(currentParam);
              return Array.isArray(x) && (x.length === 0);
            })()) {
              let i: any = 1;
              let result: any = currentParam;
...
function compileCond(node: any, env: any, options: any = {}): any {
  const expressionType: any = options['expressionType'];
  function wrap(exps: any): any {
    if (exps.length === 1) {
      return exps[0];
    } else {
      return sexpToRose([Symbol.for('begin'), ...exps]);
    }
  }
  ...
  const clauses: any = node.drop(1).slice(0, -1);
  const finalClause: any = node.last();
  const finalExp: any = taggedListP(finalClause, Symbol.for('else')) ? sexpToRose([(expressionType === 'expression') ? Symbol.for('begin') : Symbol.for('js/block'), ...finalClause.drop(1)]) : sexpToRose([Symbol.for('if'), finalClause.get(0), wrap(finalClause.drop(1))]);
  const finalExpCompiled: any = transferAndCompileComments(finalClause, compileRose(finalExp, env, options), options);
  let result: any = clauses.reduceRight(function (alternate: any, clause: any): any {
    const conditional: any = compileRose(sexpToRose([Symbol.for('if'), clause.get(0), transferComments(clause, wrap(clause.drop(1)))]), env, options);
...
function compileJsIf(node: any, env: any, options: any = {}): any {
  ...
  if (expressionType === 'expression') {
    ...
    const thenExp: any = sexpToRose([Symbol.for('js/block'), node.get(2)]);
    let elseExp: any = node.get(3);
    if (elseExp && !formp(elseExp, jsIf_, env)) {
      elseExp = sexpToRose([Symbol.for('js/block'), elseExp]);
    }
...
function compileJsTernaryOperator(node: any, env: any, options: any = {}): any {
  ...
  let elseExp: any = node.get(3) || sexpToRose(undefined);
...
function compileDefine(node: any, env: any, options: any = {}): any {
  ...
  if (Array.isArray((Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && ((): any => {
    ...
    if (inlineLispSources) {
      let sym: any = Symbol.for(functionName);
...
function compileJsOp(node: any, env: any, options: any = {}): any {
  let op: any = roseToSexp(node.get(1));
  if (typeof op === 'symbol') {
    op = op.description as string;
  }
  const logical: any = ['&&', '||'].includes(op);
  const node1: any = sexpToRose(node.drop(1), node);
  if (node.size() === 3) {
...
function compileJsFunction(node: any, env: any, options: any = {}, settings: any = {}): any {
  ...
  if (typeof ((Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && ((): any => {
    ...
    let x: any = (Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && ((): any => {
...
function compileDefineFields(node: any, env: any, options: any = {}): any {
  ...
  const objExp: any = roseToSexp(obj);
...
function compileJsDot(node: any, env: any, options: any = {}): any {
  if (node.size() > 3) {
    ...
    const propCompiled: any = (typeof roseToSexp(prop) === 'symbol') ? compileSymbol(prop, env, options) : compileExpression(prop, env, options);
    // Kludge: prevent TypeScript errors with expressions
    // like `x[y]`, where `y` is `any`-typed.
    ...
    if (!externalSymbols.includes(symbol) && env1.hasp(symbol)) {
      ...
      if (sourcep(value)) {
        ...
        if (taggedListP(exp, Symbol.for('define'))) {
          ...
          const compiledExpression: any = compileRose(sexpToRose(exp), env2, {
            ...options,
            currentModule,
...
function compileRequire(node: any, env: any, options: any = {}): any {
  ...
  if (fcommonjs) {
    if (taggedListP(xExp, Symbol.for('only-in'))) {
      return compileStatement(sexpToRose([Symbol.for('define-fields'), xExp.slice(2), [Symbol.for('js/require'), xExp[1]]]), env, options);
    } else {
      if (typeof xExp === 'string') {
        xExp = Symbol.for(xExp);
      }
      return compileStatement(sexpToRose([Symbol.for('define'), xExp, [Symbol.for('js/require'), yNode]]), env, options);
    }
  } else {
    let specifiers: any = [];
    const seen: any = [];
    ...
    if (taggedListP(xExp, Symbol.for('only-in'))) {
      ...
      specifiers = [fesModuleInterop ? new ImportDefaultSpecifier(new Identifier(xExp)) : new ImportNamespaceSpecifier(new Identifier(xExp))];
    }
  ...
  if (fcommonjs) {
    ...
    for (let exp of expressions) {
      if (taggedListP(exp, Symbol.for('all-from-out'))) {
        let name: any = Symbol.for(roseToSexp(exp.get(1)));
    ...
    return compileStatement(sexpToRose([Symbol.for('set-field!'), Symbol.for('exports'), Symbol.for('module'), [Symbol.for('js/obj'), ...properties]]), env, options);
  } else {
...
function compileJsFor(node: any, env: any, options: any = {}): any {
  const body: any = sexpToRose([Symbol.for('js/block'), ...node.drop(2)], node);
  let init: any = node.get(1, 0);
  const initExp: any = roseToSexp(init);
  const test: any = node.get(1, 1);
  const testExp: any = roseToSexp(test);
  let update: any = node.get(1, 2);
  const updateExp: any = roseToSexp(update);
  ...
  if (bindingp(initExp)) {
    sym = initExp[0];
    init = sexpToRose([Symbol.for('define'), ...init.drop(0)], init);
  } else if (formp(initExp, define_, env)) {
    sym = initExp[1];
  } else if (formp(initExp, setx_, env)) {
    sym = initExp[1];
  }
  let initCompiled: any = ((Array.isArray(initExp) && (initExp.length === 0)) || (initExp === undefined)) ? null : compileStatement(init, env, options);
  ...
  let testCompiled: any = ((Array.isArray(testExp) && (testExp.length === 0)) || (testExp === undefined)) ? null : compileExpression(test, env, options);
  function incrementp(x: any): any {
    return formp(x, add_, env) || formp(x, sub_, env);
  }
  ...
  if (incrementp(updateExp)) {
    ...
    if (sym) {
      update = sexpToRose([Symbol.for('set!'), sym, update], update);
    }
  }
  let updateCompiled: any = ((Array.isArray(updateExp) && (updateExp.length === 0)) || (updateExp === undefined)) ? null : compileStatement(update, env, options);
  ...
  const bodyCompiled: any = compileStatement(body, env, options);
...
function compileJsForIn(node: any, env: any, options: any = {}): any {
  let left: any = sexpToRose([Symbol.for('define'), node.get(1, 0, 0)], node);
  let right: any = node.get(1, 0, 1);
  const body: any = sexpToRose([Symbol.for('js/block'), ...node.drop(2)], node);
  const leftCompiled: any = compileStatement(left, env, options);
...
function compileClassHelper(node: any, env: any, options: any = {}): any {
  ...
  let id: any = hasName ? new Identifier(printEstree(compileExpression(classNameNode, env, inheritedOptions), inheritedOptions)) : null;
...
function compileMakeHashMacro(exp: any, env: any): any {
  ...
  if (assocs) {
    if ((taggedListP(assocs, Symbol.for('quasiquote')) || taggedListP(assocs, Symbol.for('quote'))) && ((): any => {
      let x: any = lastCdr((Array.isArray(assocs) && (assocs.length >= 3) && (assocs[assocs.length - 2] === Symbol.for('.')) && ((): any => {
...
function mapVisitRose(f: any, node: any, env: any = new LispEnvironment(), stack: any = [], bindings: any = new LispEnvironment()): any {
  ...
  function visitLet(node: any, stack: any, bindings: any): any {
    ...
    let sym: any = roseToSexp(node.get(0));
  ...
  if (expressionType === 'expression') {
    ...
    const casesCompiled: any = cases.map(function (x: any): any {
      ...
      if (op === Symbol.for('case')) {
        ...
        const consequentBlock: any = sexpToRose([Symbol.for('js/block'), ...consequent], x);

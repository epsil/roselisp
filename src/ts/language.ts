...
function compile(exp: any, ...args: any[]): any {
  ...
  const options: any = (args.length === 1) ? args[0] : plistToObject_(args, true);
  ...
  if (toLanguage === 'Roselisp') {
    ...
    const expressionType: any = (as === Symbol.for('return-statement')) ? 'return' : ((as === Symbol.for('statement')) ? 'statement' : 'expression');
    let caseOption: any = options['case'];
...
function compileFexprCall(node: any, env: any, options: any = {}): any {
  ...
  const quotedArgs: any = args.map(function (arg: any): any {
    return makeRose([Symbol.for('quote'), arg], arg);
  });
  const call: any = makeRose([op, ...quotedArgs], node);
...
function makeIdentifierString(str: any, options: any = {}): any {
  let result: any = str;
  let caseOption: any = options['case'] || 'none';
  if (validJsCasingStyleP(caseOption)) {
...
function compileDefineFexpr(node: any, env: any, options: any = {}): any {
  ...
  let name: any = nameAndArgs.get(0).getValue();
  const body: any = node.drop(1);
  let result: any = compileRose(transferComments(node, makeRose([Symbol.for('begin'), [Symbol.for('define'), ...body], [Symbol.for('declare'), name, [Symbol.for('ftype'), 'fexpr']]], node)), env, options);

...
function compileDot(node: any, env: any, options: any = {}): any {
  ...
  if (method === '') {
    ...
    if ((match = (((Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && ((): any => {
      ...
      const obj: any = node.get(1);
      return compileGetField(sexpToRose([Symbol.for('get-field'), fieldSym, obj]), env, options);
    } else {
    ...
    if ((match = method.match(new RegExp('^-(.*)$')))) {
      ...
      return compileGetField(sexpToRose([Symbol.for('get-field'), Symbol.for(field), obj], node), env, options);
    } else {
      // Method call:
...
function compileGetField(node: any, env: any, options: any = {}): any {
  ...
  const obj: any = node.get(2);
  const computed: any = typeof roseToSexp(field) !== 'symbol';
  return makeExpressionOrStatement(new MemberExpression((typeof roseToSexp(obj) === 'symbol') ? compileSymbol(obj, env, makeExpressionOptions(options)) : compileExpression(obj, env, options), computed ? compileExpression(field, env, options) : compileSymbol(field, env, options), computed), options);
}
...
function compileJsOptionalChaining(node: any, env: any, options: any = {}): any {
  ...
  const fieldExp: any = roseToSexp(field);
  if (Array.isArray(fieldExp)) {
    let result: any = compileRose(sexpToRose([obj, ...field.drop(0)], node), env, options);
    result.optional = true;
    return result;
  } else {
    let result: any = compileRose(sexpToRose([Symbol.for('get-field'), field, obj], node), env, options);
    result.optional = true;
    return result;
  }
}

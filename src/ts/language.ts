 ...
 */
function compileJsIife(node: any, env: any, options: any = {}, settings: any = {}): any {
  const expressionType: any = options['expressionType'];
  const f: any = node.get(1);
  const args: any = node.get(2);
  let argsList: any = args.drop(1);
  ...
  if (['statement', 'return'].includes(expressionType)) {
    const fExp: any = syntaxToDatum(f);
    let params: any = fExp[1];
    let [regularParams, restParam]: any[] = parseParamsList(params);
    if (restParam) {
      params = [...regularParams, restParam];
    }
    ...
    return compileSyntax(datumToSyntax(node, [Symbol.for('let*'), letBindingsEnv, ...letBody]), env, options);
  } else {
    return compileSyntax(datumToSyntax(node, [fapply, f, ...argsList]), env, options);
  }
}
 ...
 */
function compileArrayPushLeft(node: any, env: any, options: any = {}): any {
  // `.unshift()` returns the length of the array,
  // while `push!()` returns the list.
  return compileArrayPushHelper(datumToSyntax(false, [Symbol.for('send'), node.get(1), Symbol.for('unshift'), node.get(2)]), datumToSyntax(false, [[Symbol.for('lambda'), [Symbol.for('arr'), Symbol.for('x')], [Symbol.for('send'), Symbol.for('arr'), Symbol.for('unshift'), Symbol.for('x')], Symbol.for('arr')], node.get(1), node.get(2)]), node, env, options);
}
 ...
 */
function compileArrayPushRight(node: any, env: any, options: any = {}): any {
  // `.push()` returns the length of the array,
  // while `push-right!()` returns the list.
  return compileArrayPushHelper(datumToSyntax(false, [Symbol.for('send'), node.get(1), Symbol.for('push'), node.get(2)]), datumToSyntax(false, [[Symbol.for('lambda'), [Symbol.for('arr'), Symbol.for('x')], [Symbol.for('send'), Symbol.for('arr'), Symbol.for('push'), Symbol.for('x')], Symbol.for('arr')], node.get(1), node.get(2)]), node, env, options);
}
 ...
 */
function compileArrayPushHelper(statementExp: any, expressionExp: any, node: any, env: any, options: any = {}): any {

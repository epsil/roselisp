...
function compile(exp: any, ...args: any[]): any {
  function languageSymbolToLanguageString(sym: any): any {
  ...
  const options: any = (args.length === 1) ? args[0] : plistToObject_(args, {
    case: 'camelcase'
  });
  const from: any = options['from'] || Symbol.for('roselisp');
  const to: any = options['to'] || Symbol.for('javascript');
  const fromLanguage: any = languageSymbolToLanguageString(from);
  const toLanguage: any = languageSymbolToLanguageString(to);
  if (toLanguage === 'Roselisp') {
    ...
    const as: any = options['as'];
    const expressionType: any = (as === Symbol.for('expression')) ? 'expression' : ([Symbol.for('return-statement'), Symbol.for('return')].includes(as) ? 'return' : 'statement');
...
function decompile(exp: any, ...args: any[]): any {
  ...
  const from: any = options['from'] || Symbol.for('javascript');
  const to: any = options['to'] || Symbol.for('roselisp');
  const inheritedOptions: any = {
    ...options,
    from: from,
    to: to
...
function compileModules(modules: any, env: any, options: any = {}): any {
  ...
  for (let module of modules) {
    if (!(module instanceof Rose)) {
      module = sexpToRose(module);
    }
    moduleName = roseToSexp(module.get(1));
    ...
    if (exp.length === 0) {
      ...
      if ((typeof op === 'symbol') && env.has(op, {
        ...
        if (undefinedTypeP(opType)) {
          ...
          const inlinedNode: any = sexpToRose(inlinedExp, node);
...
function compileSexp(exp: any, env: any, options: any = {}): any {
  return compileRose(sexpToRose(exp), env, options);
}
...
function wrapInLambdaCall(exp: any): any {
  return sexpToRose([[Symbol.for('lambda'), [], exp]]);
}
...
function wrapInArrowCall(exp: any): any {
  return sexpToRose([[Symbol.for('js/arrow'), [], exp]]);
}
...
function functionCallP(exp: any, env: any): any {
  if (exp instanceof Rose) {
    return macroCallP(roseToSexp(exp), env);
  } else {
...
function defineToLambda(node: any, options: any = {}): any {
  ...
  return sexpToRose([Symbol.for('lambda'), params, ...node.drop(2)]);
}
...
function compileType(node: any, env: any, options: any = {}): any {
  let exp: any = (node instanceof Rose) ? roseToSexp(node) : node;
...
function compileApply(node: any, env: any, options: any = {}): any {
  ...
  const calleeCompiled: any = compileExpression(sexpToRose(callee), env, options);
  const argsCompiled: any = [];
  if (args.length > 0) {
    let regularArgs: any = args.slice(0, -1);
    for (let arg of regularArgs) {
      argsCompiled.push(compileExpression(sexpToRose(arg), env, options));
    }
    let restArg: any = args[args.length - 1];
    const restArgCompiled: any = compileExpression(sexpToRose(restArg), env, options);
...
function compileAtom(node: any, env: any, options: any = {}): any {
  return makeExpressionOrStatement(new Literal(roseToSexp(node)), options);
}
...
function compileColon(node: any, env: any, options: any = {}): any {
  ...
  const symExp: any = roseToSexp(sym);
  let type_: any = node.get(2);
  const typeExp: any = roseToSexp(type_);
  ...
  if (condClauses.length === 0) {
    ...
    if (roseToSexp(condClauses[condClauses.length - 1].get(0)) === Symbol.for('else')) {
      ...
      return condClauses.reduceRight(function (acc: any, x: any): any {
        return reducingF(x, acc);
      }, (expressionType === 'expression') ? new Identifier('undefined') : null);
    }
  }
}
...
function compileDefine(node: any, env: any, options: any = {}): any {
  ...
  if (Array.isArray((Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && ((): any => {
    ...
    const functionName: any = printEstree(compileSymbol(sexpToRose(nameSym), env, makeExpressionOptions(options)), options);
    const lambdaExp: any = defineToLambda(node);
    const returnType: any = (roseToSexp(node.get(2)) === Symbol.for(':')) ? roseToSexp(node.get(3)) : Symbol.for('Any');
    let params: any = roseToSexp(lambdaExp.get(1));
    ...
    let result: any;
    if (shouldCurry) {
      result = compileDefine(sexpToRose([Symbol.for('define'), nameSym, lambdaExp], node), env, options);
    } else {
    ...
    const daForm: any = transferComments(node, sexpToRose([Symbol.for('define/async'), [name, ...args], ...lambdaNode.drop(2)]));
    ...
    // Class definition.
    return compileDefineClass(sexpToRose(defineToDefineClass(exp), node), env, options);
  } else {
...
function compileDiv(node: any, env: any, options: any = {}): any {
  let exp: any = roseToSexp(node);
  if (exp.length === 1) {
    return compileExpression(sexpToRose(undefined, node), env, options);
  } else if (exp.length === 2) {
    return compileDiv(sexpToRose([Symbol.for('/'), 1, node.get(1)], node), env, options);
  } else {
    return compileBinaryExpression(node, env, options, {
      identity: 1,
      operator: '/'
...
function compileSend(node: any, env: any, options: any = {}): any {
  ...
  return makeExpressionOrStatement(new CallExpression(new MemberExpression((typeof roseToSexp(obj) === 'symbol') ? compileSymbol(obj, env, makeExpressionOptions(options)) : compileExpression(obj, env, options), compileSymbol(method, env, options), false), args.map(function (x: any): any {
    return compileExpression(x, env, options);
  })), options);
}
...
function compileFunctionCall(node: any, env: any, options: any = {}): any {
  ...
  const op: any = roseToSexp(callee);
...
function compileGreaterThan(node: any, env: any, options: any = {}): any {
  let exp: any = roseToSexp(node);
  if (exp.length < 3) {
    return compileRose(sexpToRose(true), env, options);
    ...
    return compileRose(sexpToRose(andExp), env, options);
  }
}
...
function compileBinaryExpression(node: any, env: any, options: any = {}, settings: any = {}): any {
  ...
  if (operands.length === 0) {
    ...
    return makeExpressionOrStatement(compileRose(sexpToRose(identity), env, options), options);
  ...
  if (regularArgs) {
    for (let arg of regularArgs) {
      if (colonFormP(arg)) {
        ...
        let result: any = ((arg.length === 4) ? new AssignmentPattern(new Identifier(printEstree(compileExpression(sexpToRose(sym), env1, inheritedOptions), inheritedOptions)), compileExpression(sexpToRose((Array.isArray(arg) && (arg.length >= 3) && (arg[arg.length - 2] === Symbol.for('.')) && ((): any => {
          const x: any = lastCdr(arg);
          return Array.isArray(x) && (x.length === 0);
        })()) ? ((): any => {
          let i: any = 3;
          ...
          return result;
        })() : arg[3]), env1, inheritedOptions)) : new Identifier(printEstree(compileExpression(sexpToRose(sym), env1, inheritedOptions), inheritedOptions))).setType(compileType(typ, env1, options));
        params.push(result);
      } else if (Array.isArray(arg)) {
        makeTypeBinding(env1, arg[0], Symbol.for('Any'), langFilter);
        params.push(new AssignmentPattern(new Identifier(printEstree(compileExpression(sexpToRose(arg[0]), env1, inheritedOptions), inheritedOptions)), compileExpression(sexpToRose((Array.isArray(arg) && (arg.length >= 3) && (arg[arg.length - 2] === Symbol.for('.')) && ((): any => {
          const x: any = lastCdr(arg);
          return Array.isArray(x) && (x.length === 0);
        })()) ? ((): any => {
          let i: any = 1;
        ...
        params.push(new Identifier(printEstree(compileExpression(sexpToRose(arg), env1, inheritedOptions), inheritedOptions)));
      }
    }
  }
  if (restArg) {
    makeTypeBinding(env1, restArg, Symbol.for('Any'), langFilter);
    params.push(new RestElement(compileExpression(sexpToRose(restArg), env1, inheritedOptions)));
  }
  let bodyStatements: any = node.drop(2);
  if ((bodyStatements.length > 0) && (roseToSexp(bodyStatements[0]) === Symbol.for(':'))) {
  ...
  if (expressionType === 'expression') {
    ...
    const defineNodes: any = letNodes.map(function (x: any): any {
      ...
      if (Array.isArray(exp)) {
        ...
        return sexpToRose([Symbol.for('define'), x.get(0), x.get(1)], x);
      } else {
        const sym: any = exp;
        if (!makeBlock && env.has(sym, {
          filter: langFilter
        ...
        return sexpToRose([Symbol.for('define'), x], x);
      }
    });
    const env1: any = makeBlock ? extendEnvironment(new LispEnvironment(), env) : env;
    let result: any = compileRose(sexpToRose([makeBlock ? Symbol.for('js/block') : Symbol.for('begin'), ...defineNodes, ...bodyNodes], node), env1, inheritedOptions);
    return result;
  }
}
  ...
  if (expressionType === 'expression') {
    ...
    const defineNodes: any = letNodes.map(function (x: any): any {
      ...
      if (typeof exp === 'symbol') {
        ...
        return sexpToRose([Symbol.for('define'), x]);
      } else {
        const variables: any = roseToSexp(x.get(0));
        ...
        return sexpToRose([Symbol.for('define-values'), x.get(0), x.get(1)], x);
      }
    });
    const env1: any = makeBlock ? extendEnvironment(new LispEnvironment(), env) : env;
    let result: any = compileRose(sexpToRose([makeBlock ? Symbol.for('js/block') : Symbol.for('begin'), ...defineNodes, ...bodyNodes], node), env1, inheritedOptions);
...
function compileDefineValues(node: any, env: any, options: any = {}): any {
  ...
  const variables: any = roseToSexp(node.get(1));
  ...
  if (roseToSexp(expression) === Symbol.for(':hole-marker')) {
    holeMarker = roseToSexp(node.get(3));
    expression = node.get(4);
  }
  if (typeof variables === 'symbol') {
    declaratorId = new Identifier(printEstree(compileSymbol(sexpToRose(variables), env, inheritedOptions), inheritedOptions));
    ...
    varDecls = regularVars.map(function (x: any): any {
      if (x === holeMarker) {
        ...
        return new Identifier(printEstree(compileSymbol(sexpToRose(x), env, inheritedOptions), inheritedOptions));
      }
    });
    if (restVar) {
      makeTypeBinding(env, restVar, Symbol.for('Any'), langFilter);
      varDecls.push(new RestElement(new Identifier(printEstree(compileSymbol(sexpToRose(restVar), env, inheritedOptions), inheritedOptions))));
    }
...
function compileSetValues(node: any, env: any, options: any = {}): any {
  ...
  let left: any;
  let right: any;
  declaration = compileDefineValues(sexpToRose([Symbol.for('define-values'), ...node.drop(1)], node), env, inheritedOptions);
...
function compileLetFields(node: any, env: any, options: any = {}): any {
  ...
  if (expressionType === 'expression') {
    ...
    const defineNodes: any = letNodes.map(function (x: any): any {
      ...
      const fieldsExp: any = roseToSexp(fields);
      ...
      return sexpToRose([Symbol.for('define-fields'), fields, obj], x);
    });
    const env1: any = makeBlock ? extendEnvironment(new LispEnvironment(), env) : env;
    let result: any = compileRose(sexpToRose([makeBlock ? Symbol.for('js/block') : Symbol.for('begin'), ...defineNodes, ...bodyNodes], node), env1, inheritedOptions);
    return result;
...
function compileDefineFields(node: any, env: any, options: any = {}): any {
  ...
  const expressionStatement: any = compileSetFields(sexpToRose([Symbol.for('set!-fields'), fields, obj], node), env, makeStatementOptions(options));
...
function compileFexprCall(node: any, env: any, options: any = {}): any {
  const op: any = node.get(0);
  const args: any = node.drop(1);
  const quotedArgs: any = args.map(function (arg: any): any {
    return sexpToRose([Symbol.for('quote'), arg], arg);
  });
  const call: any = sexpToRose([op, ...quotedArgs], node);
...
function macroexpandstar1(exp: any, env: any = undefined): any {
  ...
  let expansion: any = exp;
  let expanded: any = false;
  exp = (node instanceof Rose) ? roseToSexp(node) : node;
  ...
  return [(node instanceof Rose) ? sexpToRose(expansion, node) : expansion, expanded];
}
...
function compileDot(node: any, env: any, options: any = {}): any {
  ...
  if (method === '') {
    ...
    if ((match = (((Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && ((): any => {
      ...
      const obj: any = node.get(1);
      return compileJsDot(sexpToRose([Symbol.for('js/.'), obj, fieldSym]), env, options);
    } else {
    ...
    if ((match = method.match(new RegExp('^-(.*)$')))) {
      ...
      return compileJsDot(sexpToRose([Symbol.for('js/.'), obj, Symbol.for(field)], node), env, options);
    } else {
      // Method call:
      // `(.foo bar ...)` = `(send bar foo ...)`.
      return compileSend(sexpToRose([Symbol.for('send'), obj, Symbol.for(method), ...node.drop(2)], node), env, options);
    }
  }
}
...
function compileJsDot(node: any, env: any, options: any = {}): any {
  if (node.size() > 3) {
    return compileJsDot(sexpToRose(node.drop(2).reduce(function (obj: any, prop: any): any {
      return [Symbol.for('js/.'), obj, prop];
    }, node.get(1)), node), env, options);
  } else {
    const obj: any = node.get(1);
    const prop: any = node.get(2);
    const computed: any = typeof roseToSexp(prop) !== 'symbol';
    return makeExpressionOrStatement(new MemberExpression((typeof roseToSexp(obj) === 'symbol') ? compileSymbol(obj, env, makeExpressionOptions(options)) : compileExpression(obj, env, options), computed ? compileExpression(prop, env, options) : compileSymbol(prop, env, options), computed), options);
  }
}
...
function compileJsOptionalChaining(node: any, env: any, options: any = {}): any {
  if (node.size() > 3) {
    return compileJsOptionalChaining(sexpToRose(node.drop(2).reduce(function (obj: any, prop: any): any {
      return [Symbol.for('js/?.'), obj, prop];
    }, node.get(1)), node), env, options);
  } else if (node.size() === 2) {
    ...
    let result: any = Array.isArray(roseToSexp(field)) ? compileExpression(sexpToRose([obj, ...field.drop(0)], node), env, options) : compileExpression(sexpToRose([Symbol.for('js/.'), obj, field], node), env, options);
...
function compileSetField(node: any, env: any, options: any = {}): any {
  ...
  return compileRose(sexpToRose([Symbol.for('set!'), [Symbol.for('get-field'), field, obj], val], node), env, options);
}
...
function compileBegin(node: any, env: any, options: any = {}): any {
  ...
  for (let i: any = 0; i < _end; i++) {
    let exp: any = roseToSexp((body as any)[i]);
    ...
    if (!externalSymbols.includes(symbol) && env.has(symbol)) {
      ...
      if (sourcep(value)) {
        ...
        if (taggedListP(exp, Symbol.for('define'))) {
          ...
          const compiledExpression: any = compileRose(sexpToRose(exp), env1, {
            ...options,
...
function compileProvide(node: any, env: any, options: any = {}): any {
  ...
  for (let x of expressions) {
    if (taggedListP(roseToSexp(x), Symbol.for('all-from-out'))) {
    ...
    for (let x of otherExpressions) {
      ...
      if (taggedListP(exp, Symbol.for('rename-out'))) {
        for (let pair of exp.slice(1)) {
          ...
          if (typeof x1 === 'symbol') {
            x1 = printEstree(compileSymbol(sexpToRose(x1), env, options, {
              literalSymbol: true
            }), options);
          }
          if (typeof x2 === 'symbol') {
            x2 = printEstree(compileSymbol(sexpToRose(x2), env, options, {
              literalSymbol: true
            }), options);
          }
          if (!seen.includes(x2)) {
...
function compileQuote(node: any, env: any, options: any = {}): any {
  ...
  if (Array.isArray((Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && ((): any => {
    ...
    result = compileExpression(sexpToRose([Symbol.for('list'), ...((Array.isArray(exp) && (exp.length >= 3) && (exp[exp.length - 2] === Symbol.for('.')) && ((): any => {
      const x: any = lastCdr(exp);
      return Array.isArray(x) && (x.length === 0);
    })()) ? ((): any => {
      let i: any = 1;
...
function compileQuasiquoteHelper(node: any, env: any, options: any = {}): any {
  let exp: any = roseToSexp(node);
  if (!Array.isArray(exp)) {
    return compileExpression(sexpToRose([Symbol.for('quote'), exp]), env, options);
  } else {
    return new ArrayExpression(node.getNodes().map(function (x: any): any {
      let exp: any = roseToSexp(x);
      if (taggedListP(exp, Symbol.for('quasiquote'))) {
        return compileQuote(sexpToRose([Symbol.for('quote'), exp]), env, makeExpressionOptions(options));
...
function compileRequire(node: any, env: any, options: any = {}): any {
  ...
  let xExp: any = roseToSexp(xNode);
  const yNode: any = node.get(2) || xNode;
  let yExp: any = roseToSexp(yNode);
  let specifiers: any = [];
  const seen: any = [];
  let src: any = null;
  if (taggedListP(xExp, Symbol.for('only-in'))) {
    for (let x of xNode.drop(2)) {
      ...
      if (Array.isArray(exp)) {
        ...
        let x2Str: any = x2;
        if (typeof x1 === 'symbol') {
          x1Str = printEstree(compileSymbol(sexpToRose(x1), env, options, {
            literalSymbol: true
          }), options);
        }
        if (typeof x2 === 'symbol') {
          x2Str = printEstree(compileSymbol(sexpToRose(x2), env, options, {
            literalSymbol: true
          }), options);
        }
    ...
    if (typeof xExp === 'symbol') {
      xExp = printEstree(compileSymbol(sexpToRose(xExp), env, options, {
        literalSymbol: true
      }), options);
    }
  ...
  if (typeof yExp === 'symbol') {
    yExp = printEstree(compileSymbol(sexpToRose(yExp), env, options, {
      literalSymbol: true
    }), options);
  }
...
function compileSet(node: any, env: any, options: any = {}): any {
  ...
  const symExp: any = roseToSexp(symNode);
  let valNode: any = node.get(2);
  let valExp: any = roseToSexp(valNode);
  ...
  if (formp(valExp, add1_, env) && (((Array.isArray(valExp) && (valExp.length >= 3) && (valExp[valExp.length - 2] === Symbol.for('.')) && ((): any => {
    ...
    result = new AssignmentExpression('=', (typeof roseToSexp(symNode) === 'symbol') ? compileSymbol(symNode, env, makeExpressionOptions(options)) : compileExpression(symNode, env, options), compileExpression(valNode, env, options));
  }
  return makeExpressionOrStatement(result, options);
}
...
function compileString(node: any, env: any, options: any = {}): any {
  ...
  if (str.match(new RegExp('\\n'))) {
    ...
    if (lines.length <= 1) {
      ...
      return compileRose(transferComments(node, sexpToRose([Symbol.for('string-append'), ...lines], node)), env, options);
    }
  } else {
    return compileAtom(node, env, options);
  }
  ...
  if (!(quotedSymbol || literalSymbol || (env.has(exp) && variableTypeP(compilationMappingEnvironment.getType(env.get(exp)))))) {
    if (shouldInlineP(exp, env, options)) {
      if (currentModule) {
        ...
        return makeExpressionOrStatement(compileExpression(sexpToRose(makeInlinedValue(exp, env, options)), env, options), options);
      }
    }
  }
...
function compileSymbol(node: any, env: any, options: any = {}, settings: any = {}): any {
  ...
  if (quotedSymbolOption) {
    return compileExpression(sexpToRose([Symbol.for('string->symbol'), str]), env, options);
...
function compileFor(node: any, env: any, options: any = {}): any {
  ...
  if (formp(valsExp, range_, env)) {
    ...
    if (Array.isArray(start) || Array.isArray(end) || Array.isArray(step)) {
      ...
      return compileRose(sexpToRose([Symbol.for('let'), [...(Array.isArray(start) ? [[startVar, start]] : []), ...(Array.isArray(end) ? [[endVar, end]] : []), ...(Array.isArray(step) ? [[stepVar, step]] : [])], [Symbol.for('for'), [[symExp, [Symbol.for('range'), Array.isArray(start) ? startVar : start, Array.isArray(end) ? endVar : end, ...(step ? (Array.isArray(step) ? [stepVar] : [step]) : [])]]], ...bodyNodes]], node), env, options);
    }
    // Otherwise, proceed to create a `for` loop.
    step = step || 1;
    const init: any = compileStatement(sexpToRose([Symbol.for('define'), symExp, start]), env, inheritedOptions);
    let test: any;
    let update: any;
    if (Number.isFinite(step)) {
      if (step < 0) {
        test = compileExpression(sexpToRose([Symbol.for('>'), symExp, end]), env, inheritedOptions);
        update = compileStatement(sexpToRose([Symbol.for('set!'), symExp, [Symbol.for('-'), symExp, Math.abs(step)]]), env, inheritedOptions);
      } else {
        test = compileExpression(sexpToRose([Symbol.for('<'), symExp, end]), env, inheritedOptions);
        update = compileStatement(sexpToRose([Symbol.for('set!'), symExp, [Symbol.for('+'), symExp, step]]), env, inheritedOptions);
      }
    } else {
      test = compileExpression(sexpToRose([Symbol.for('if'), [Symbol.for('<'), step, 0], [Symbol.for('>'), symExp, end], [Symbol.for('<'), symExp, end]]), env, inheritedOptions);
      update = compileStatement(sexpToRose([Symbol.for('set!'), symExp, [Symbol.for('+'), symExp, step]]), env, inheritedOptions);
    }
    ...
    let left: any = compileExpression(sexpToRose([Symbol.for('define'), symExp]), env, inheritedOptions);
...
function compileClass(node: any, env: any, options: any = {}): any {
  ...
  const className: any = roseToSexp(classNameNode);
  ...
  let bodyExp: any = roseToSexp(bodyNode);
  const env1: any = extendEnvironment(new LispEnvironment(), env);
  ...
  if (Array.isArray(bodyExp[0]) && !formp(bodyExp[0], define_, env1)) {
    ...
    const superClasses: any = roseToSexp(superClassesNode);
    bodyNode = sliceRose(bodyNode, 1);
    bodyExp = roseToSexp(bodyNode);
    if (superClasses.length > 0) {
      superClass = new Identifier(printEstree(compileExpression(sexpToRose(superClasses[0]), env1, inheritedOptions), inheritedOptions));
    }
  }
...
function compileJsObj(node: any, env: any, options: any = {}): any {
  ...
  for (let i: any = 1; i < _end; i = i + 2) {
    ...
    const keyValue: any = roseToSexp(keyNode);
    ...
    const computed: any = typeof roseToSexp(keyNode) !== 'string';
    let match: any;
...
function compileJsTaggedTemplate(node: any, env: any, options: any = {}): any {
  ...
  const strExp: any = roseToSexp(str);
...
function compileJsTry(node: any, env: any, options: any = {}): any {
  ...
  const block: any = wrapInBlockStatementSmart(compileStatementOrReturnStatement(sexpToRose([Symbol.for('begin'), ...bodyExps]), env, options));
  let handler: any = null;
  if (catchClause) {
    // TODO: Permit destructuring.
    let param: any = catchClause.get(1);
    const paramExp: any = roseToSexp(param);
    const paramCompiled: any = (paramExp === Symbol.for('_')) ? null : compileExpression(param, env, options);
    const body: any = sexpToRose([Symbol.for('begin'), ...catchClause.drop(2)]);
  ...
  const finalizer: any = finallyClause ? wrapInBlockStatementSmart(compileStatement(sexpToRose([Symbol.for('begin'), ...finallyClause.drop(1)]), env, options)) : null;
...
function compilePushLeft(node: any, env: any, options: any = {}): any {
  ...
  // returns the list.
  return compilePushHelper(sexpToRose([Symbol.for('send'), node.get(1), Symbol.for('unshift'), node.get(2)]), sexpToRose([[Symbol.for('lambda'), [Symbol.for('lst'), Symbol.for('x')], [Symbol.for('send'), Symbol.for('lst'), Symbol.for('unshift'), Symbol.for('x')], Symbol.for('lst')], node.get(1), node.get(2)]), node, env, options);
}
...
function compilePushRight(node: any, env: any, options: any = {}): any {
  ...
  // returns the list.
  return compilePushHelper(sexpToRose([Symbol.for('send'), node.get(1), Symbol.for('push'), node.get(2)]), sexpToRose([[Symbol.for('lambda'), [Symbol.for('lst'), Symbol.for('x')], [Symbol.for('send'), Symbol.for('lst'), Symbol.for('push'), Symbol.for('x')], Symbol.for('lst')], node.get(1), node.get(2)]), node, env, options);
}
...
function compilePushHelper(statementExp: any, expressionExp: any, node: any, env: any, options: any = {}): any {
  ...
  } else if (typeof roseToSexp(node.get(1)) === 'symbol') {
 ...
 */
function compileDefineFexpr(node: any, env: any, options: any = {}): any {
  const languageEnv: any = options['languageEnvironment'];
  function langFilter(x: any): any {
    return x !== languageEnv;
  }
  ...
  let name: any = roseToSexp(nameAndArgs.get(0));
  const body: any = node.drop(1);
  let result: any = compileRose(transferComments(node, sexpToRose([Symbol.for('begin'), [Symbol.for('define'), ...body], [Symbol.for('declare'), name, [Symbol.for('ftype'), 'fexpr']]], node)), env, options);
...
function compileDefineMacro(node: any, env: any, options: any = {}): any {
  ...
  let result: any = compileRose(transferComments(node, sexpToRose([Symbol.for('begin'), [Symbol.for('define'), [name, ...args], ...body], [Symbol.for('declare'), name, [Symbol.for('ftype'), 'macro']]], node)), env, options);
 ...
 */
function compileDeclareFexpr(node: any, env: any, options: any = {}): any {
  const languageEnv: any = options['languageEnvironment'];
  function langFilter(x: any): any {
    return x !== languageEnv;
  }
  langFilter.fsource = [Symbol.for('define'), [Symbol.for('lang-filter'), Symbol.for('x')], [Symbol.for('not'), [Symbol.for('eq?'), Symbol.for('x'), Symbol.for('language-env')]]];
  let name: any = roseToSexp(node.get(1));
  makeTypeBinding(env, name, [Symbol.for('fexpr->'), Symbol.for('Any'), Symbol.for('*'), Symbol.for('Any')], langFilter);
  return emptyProgram();
}
...
function defineMacro_(exp: any, env: any): any {
  ...
  const f: any = eval_([Symbol.for('begin'), fExp, name], env);
 ...
 */
function declareFexpr_(exp: any, env: any): any {
  const [x]: any[] = exp.slice(1);
  return compileSexp(exp, env, currentCompilationOptions());
}
...
function getField_(exp: any, env: any): any {
  const [field, obj]: any[] = exp.slice(1);
...
function mapVisitRose(f: any, node: any, env: any = new LispEnvironment(), stack: any = [], bindings: any = new LispEnvironment()): any {
  ...
  function visitLet(node: any, stack: any, bindings: any): any {
    ...
    const sym: any = roseToSexp(node.get(0));
    const letBindingsEnv: any = node.get(1);
    const body: any = node.drop(2);
    for (let letBinding of roseToSexp(letBindingsEnv)) {
    ...
    if (!((letBindingsEnv === visitedLetBindingsEnv) && (body === visitedBody))) {
      result = transferComments(node, sexpToRose([sym, visitedLetBindingsEnv, ...visitedBody]));
    }
    return f(result, stack, bindings);
  }
  ...
  function visitLetValues(node: any, stack: any, bindings: any): any {
    ...
    const visitedLetBindingsEnv: any = visitFormsNodeWith(function (x: any): any {
      ...
      let val: any = x.get(1);
      const idsExp: any = roseToSexp(ids);
      ...
      if (!((visitedIds === ids) && (visitedVal === val))) {
        xResult = transferComments(x, sexpToRose([visitedIds, visitedVal]));
      }
      return xResult;
  ...
  function visitCond(node: any, stack: any, bindings: any): any {
    ...
    if (visitedClauses !== clauses) {
      result = transferComments(node, sexpToRose([sym, ...visitedClauses]));
    }
    return f(result, stack, bindings);
  }
  ...
  function visitLambda(node: any, stack: any, bindings: any): any {
    ...
    const paramsExp: any = roseToSexp(params);
    const body: any = node.drop(2);
    ...
    if (!((params === visitedParams) && (body === visitedBody))) {
      result = transferComments(node, sexpToRose([sym, visitedParams, ...visitedBody]));
    }
    return f(result, stack, bindings);
  }
  ...
  function visitDefine(node: any, stack: any, bindings: any): any {
    let result: any = node;
    const defineSym: any = roseToSexp(node.get(0));
    let id: any = node.get(1);
    const idExp: any = roseToSexp(id);
    ...
    if (!((id === visitedId) && (body === visitedBody))) {
      result = transferComments(node, sexpToRose([defineSym, visitedId, ...visitedBody]));
    }
    return f(result, stack, bindings);
  }
  ...
  function visitDefmacro(node: any, stack: any, bindings: any): any {
    let result: any = node;
    const defmacroSym: any = roseToSexp(node.get(0));
    let id: any = node.get(1);
    const idSym: any = roseToSexp(id);
    ...
    if (!((id === visitedId) && (params === visitedParams) && (body === visitedBody))) {
      result = transferComments(node, sexpToRose([defmacroSym, visitedId, visitedParams, ...visitedBody]));
    }
  ...
  function visitDefineMacro(node: any, stack: any, bindings: any): any {
    let result: any = node;
    const defineMacroSym: any = roseToSexp(node.get(0));
    const nameAndArgs: any = node.get(1);
    const nameAndArgsExp: any = roseToSexp(nameAndArgs);
    const idSym: any = nameAndArgsExp[0];
    let id: any = sexpToRose(idSym, nameAndArgs);
    const paramsExp: any = cdr(nameAndArgsExp);
    let params: any = sexpToRose(paramsExp, nameAndArgs);
    const body: any = node.drop(2);
    makeTypeBinding(bindings, idSym, [Symbol.for('macro->'), Symbol.for('Any'), Symbol.for('*'), Symbol.for('Any')]);
    let bindings2: any = extendEnvironment(new LispEnvironment(), bindings);
    if (typeof paramsExp === 'symbol') {
    ...
    const visitedBody: any = visitFormsList(body, [...stack, node], bindings2);
    if (!((id === visitedId) && (params === visitedParams) && (body === visitedBody))) {
      result = transferComments(node, sexpToRose([defineMacroSym, cons(visitedId, visitedParams), ...visitedBody]));
    }
    result = f(result, stack, bindings);
    makeTypeBinding(bindings, idSym, [Symbol.for('macro->'), Symbol.for('Any'), Symbol.for('*'), Symbol.for('Any')]);
    return result;
  ...
  function visitQuasiquote(node: any, stack: any, bindings: any): any {
    function visitQuasiquoteForm(node: any, stack: any, bindings: any): any {
      ...
      if (val !== visitedVal) {
        result = transferComments(node, sexpToRose([sym, visitedVal]));
      }
  ...
  const f1: any = function (x: any, stack: any, bindings: any): any {
    {
      ...
      if (result === exp) {
        ...
        return sexpToRose(result, x);
      }
    }
  };
  const isRose: any = exp instanceof Rose;
  let node: any = isRose ? exp : sexpToRose(exp);
...
function compileJsSwitch(node: any, env: any, options: any = {}): any {
  ...
  if (expressionType === 'expression') {
    ...
    const casesCompiled: any = cases.map(function (x: any): any {
      const op: any = roseToSexp(x.get(0));
      ...
      if (op === Symbol.for('case')) {
        ...
        consequentCompiled = [compileStatementOrReturnStatement(sexpToRose([Symbol.for('js/block'), ...consequent]), env, options)];
      } else {
        testCompiled = null;
...
function fieldBoundP_(exp: any, env: any): any {
  ...
  const prop: any = makeIdentifierString(id.description as string, currentCompilationOptions());
...
function optimizeSexp(exp: any, env: any): any {
  if (rosep(exp)) {
    ...
    return roseToSexp(optimizeRose(sexpToRose(exp), env));
  }
}
  ...
  makeHeaderNode(nodes: any = []): any {
    ...
    if (length(nodes) > 0) {
      ...
      if (length(headerCommentStrings) > 0) {
        ...
        const headerNode: any = sexpToRose(headerExp);
      ...
      if (definitionp(exp) || macroDefinitionP(exp)) {
        ...
        moduleEnv.setLocal(name, thunk(function (): any {
          ...
          try {
            ...
            result = eval_(beginExp, moduleInterpretationEnv);
          } catch (e) {

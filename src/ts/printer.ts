...
function attachComments(result: any, node: any, options: any = {}): any {
  ...
  const _end: any = comments.length;
  for (let i: any = 0; i < _end; i++) {
...
function printDocToDocList(doc: any, options: any = {}): any {
  ...
  if (dtype === 'string') {
    ...
    for (let i: any = 0; i < _end; i++) {
      const current: any = (contentsPrinted as any)[i];
...
function printIdentifier(node: any, options: any = {}): any {
  ...
  return [getEstreeField('name', node), ((toLanguage === 'typescript') && type_) ? [getEstreeField('optional', node) ? '?:' : ':', space, printNode(type_, options)] : empty];
}

...
function testNodeRepl(exp: any): any {
  ...
  for (let clause of clauses) {
    let expected: any = eval(clause[0]);
    if (clause[1] !== '_') {
...
function testRoselispRepl(exp: any, options: any = {}): any {
  ...
  const clauses: any = parseReplForm(exp);
  for (let clause of clauses) {
    let expected: any = interpret(clause[0], testEnv);
...
function testMacro(exp: any, env: any): any {
  ...
  function promptp(exp: any): any {
    return (exp === Symbol.for('>')) || onlyPromptP(exp) || xitPromptP(exp);
  }
  function onlyPromptP(exp: any): any {
    return [Symbol.for('only>'), Symbol.for('it.only>')].includes(exp);
  }
  function xitPromptP(exp: any): any {
    return exp === Symbol.for('xit>');
  }
  let replOption: any = false;
  let group: any = [];
  const groups: any = [];
  let only: any = false;
  let i: any = 0;
  let offset: any = 3;
  const len: any = body.length;
  while (i < len) {
    const exp: any = (body as any)[i];
    if (exp === Symbol.for(':repl')) {
      offset = 2;
      replOption = body[i + 1];
    } else if (exp === Symbol.for(':describe')) {
      offset = 2;
      let description: any = body[i + 1];
      if (group.length > 0) {
        groups.push(group);
        group = [];
      }
      ...
      if (Array.isArray(expression) && (expression.length >= 2) && (expression[0] === Symbol.for('describe'))) {
        if (group.length > 0) {
          groups.push(group);
          group = [];
        }
        let description: any = expression[1];
        group.push(description);
      } else if (Array.isArray(expression) && (expression.length >= 1) && (expression[0] === Symbol.for('only'))) {
        only = true;
      } else {
        const f: any = xitPromptP(prompt) ? [Symbol.for('xit')] : ((only || onlyPromptP(prompt)) ? [Symbol.for('send'), Symbol.for('it'), Symbol.for('only')] : [Symbol.for('it')]);
        let description: any = '';
        let actual: any = undefined;
        if (taggedListP(expression, Symbol.for('it'))) {
          description = expression[1];
          actual = (expression.length > 3) ? [Symbol.for('begin'), ...expression.slice(2)] : expression[2];
        } else {
          description = printSexp(expression);
          actual = expression;
        }
        const test: any = ((expected === Symbol.for('_')) && !taggedListP(expression, Symbol.for('it'))) ? actual : [...f, description, [Symbol.for('fn'), [], ...(replOption ? [[Symbol.for('test-repl'), [Symbol.for('quote'), [Symbol.for('roselisp'), prompt, actual, expected]]]] : ((expected === Symbol.for('_')) ? (taggedListP(actual, Symbol.for('begin')) ? actual.slice(1) : [actual]) : [[Symbol.for('assert-equal'), actual, expected]]))]];
        group.push(test);
        only = false;
      }
    }

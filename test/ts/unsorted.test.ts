...
describe('for-each', function (): any {
  return xit('(compile \'(for-each (lambda (x) x) lst))', function (): any {
    return testRepl([Symbol.for('roselisp'), Symbol.for('xit>'), [Symbol.for('compile'), [Symbol.for('quote'), [Symbol.for('for-each'), [Symbol.for('lambda'), [Symbol.for('x')], Symbol.for('x')], Symbol.for('lst')]]], 'lst.forEach(function (x) {\n' +
      '  return x;\n' +
      '});']);
  });
});
